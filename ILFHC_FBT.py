...
class InertialLiftForceHelperFBT(object):
    """This class helps with parsing/utilising computed inertial lift  
    force and migration data for a neutrally buoyant spherical  
    particle suspended in flow through curved ducts having an asymmetric
    flat-bottomed trapezoidal cross-section having aspect ratio 4:1
    (measured as the width divided by the average/centre height).
    ...
    'FBT_data/utrap4x1_lift_data.npz'
    ...
    Brendan Harding, last updated 2021."""
    def __init__(self,a,R,ar=4,D=0):
        ...
        (or equivalently, the height in the middle) must be 4 (currently).
        The trapezoid delta/slope (D) is the relative difference in height at
        the right side compared to the centre, it may be 0.0,0.05,0.1,0.2,0.3,0.4.
    ...
    def _load_cs_data(self):
        ...
        if self._ar==4:
            ...
            self._raw_data = np.load('FBT_data/utrap4x1_lift_data.npz')
        else:
            raise ValueError("The requested aspect ratio is not available "+\
                             "(only 4 is currently available)")
        return
    def _update_Da_data(self):
        ...
        if len(Dis)==0:
            raise ValueError("The requested trapezoid delta/slope is not available "+\
                             "(only 0.0,0.05,0.1,0.2,0.3,0.4 are currently available)")
        ...
        # Create a NaN mask
        S = Da_data[0,0,rf[0]:rf[-1]+1,zf[0]:zf[-1]+1]
        Z = Da_data[0,1,rf[0]:rf[-1]+1,zf[0]:zf[-1]+1]
        self._nan_mask = np.ones(Z.shape)
        self._nan_mask[Z>0.975/self._a-1+2*self._D*S/self._ar] = np.nan # 0.95 or 0.975...
        #self._nan_mask[Z<-0.95/self._a+1] = np.nan # do I need to mask these, don't think so
        return
    ...
    def _setup_interpolants(self):
        ...
        nhHM,nhHR,nhW = (1.0-self._a)/self._a,(1.0+2*self._D-self._a)/self._a,(self._ar-self._a)/self._a
        bbox = [-nhW,nhW,-nhHM,nhHR]
    ...
    def _plot_DaR_component(self,ind,apply_mask=True):
        ...
        plt.colorbar()
        nhHL,nhHR,nhW,nhHM = (1.0-2*self._D)/self._a,(1.0+2*self._D)/self._a,self._ar/self._a,1.0/self._a
        plt.plot([-nhW+1,nhW-1,nhW-1,-nhW+1,-nhW+1],[-nhHM+1,-nhHM+1,nhHR-1,nhHL-1,-nhHM+1],'r--')
        plt.plot([-nhW,nhW,nhW,-nhW,-nhW],[-nhHM,-nhHM,nhHR,nhHL,-nhHM],'k-',lw=1)
        plt.xlim(-nhW,nhW)
        plt.ylim(-nhHM,max(nhHR,nhHL))
    ...
    def check_particle_bounds(self,x,z):
        ...
        S = 2*self._D/self._ar
        zmin = -1.0/self._a+1.0
        zmax = 1.0/self._a-1.0+S*x
        #return (np.abs(x)<xmax) and (z>=zmin) and (z<=zmax) # a bit crude (i.e. not entirely correct)
        return (np.abs(x)<xmax) and (z>=zmin) and (zmax-z>=(1.0+S**2)**0.5) # better
